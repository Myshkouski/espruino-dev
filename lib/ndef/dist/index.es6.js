...
/**
  ...
  */
const encode = (text, lang, encoding) => {
...
const record = (tnf = data.TNF_EMPTY, type = [], id = [], payload = [], value) => {
 ...
 * @id byte[] (optional)
 */
const textRecord = (text, languageCode, id = []) =>
  record(data.TNF_WELL_KNOWN, data.RTD_TEXT, id, encode(text, languageCode));
/**
 ...
 * @id byte[] (optional)
 */
const uriRecord = (uri, id = []) =>
  record(data.TNF_WELL_KNOWN, data.RTD_URI, id, encode$1(uri));
 ...
 * @id byte[] (optional)
 */
const absoluteUriRecord = (uri, payload = [], id = []) =>
  record(data.TNF_ABSOLUTE_URI, uri, id, payload);
...
* @id byte[] (optional)
*/
const mimeMediaRecord = (mimeType, payload = [], id = []) =>
  record(data.TNF_MIME_MEDIA, mimeType, id, payload);
...
* @id byte[] (optional)
*/
const smartPoster = (ndefRecords, id, payload) => {
  payload = [];
...
*
*/
const androidApplicationRecord = packageName =>
  record(data.TNF_EXTERNAL_TYPE, 'android.com:pkg', [], packageName);
...
*/
const encodeMessage = ndefRecords => {
  let encoded = [],
      tnf_byte,
      record_type,
      payload_length,
...
*  See NFC Data Exchange Format (NDEF) Specification Section 3.2 RecordLayout
*/
const encodeTnf = (mb, me, cf, sr, il, tnf, value = tnf) => {
  if (mb) {
    value = value | 0x80;
  }
...
export { record, textRecord, uriRecord, absoluteUriRecord, mimeMediaRecord, smartPoster, emptyRecord, androidApplicationRecord, encodeMessage, decodeMessage, decodeTnf, encodeTnf, isType };
