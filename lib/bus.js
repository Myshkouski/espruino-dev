...
function _parse(chunk) {
  ...
  for(;currentChunkIndex < chunk.length; currentChunkIndex ++) {
      ...
      for(let watchingIndex in watching) {
        ...
        try {
          incoming.push({
            ...
            byteIndex: 0,
            length: 0,
            from: this._busState.length,
            to: this._busState.length
          })
        } catch(err) {
          this.emit('error', err)
        }
    ...
    for(incomingIndex = 0; incomingIndex < incoming.length; isEqual = false) {
      ...
      const expected = incomingI.currentPattern[incomingI.byteIndex]
      ...
      if(expected === undefined || expected === chunk[currentChunkIndex]) {
        ...
        try {
          isEqual = !!expected.call(this, chunk[currentChunkIndex], incomingI.length, { at: this._busState.at }/*frame.slice(-incomingI.length - 1)*/)
        } catch(err) {
          isEqual = false
          this.emit('error', err)
        }
          ...
          if(++ incomingI.patternIndex >= incomingI.patterns.length) {
            try {
              incomingI.callback.call(
                this,
                this._busState.buffer(incomingI.from, incomingI.to)
