...
const NfcBus = {
  ...
  write(startBlock, chunk) {
    const blockOffset = (startBlock + 1) % 4
    if(!blockOffset) {
      throw new Error('Cannot start with trailer block')
    }

    const dataBlocks = []

    for(const blockLength = 16; offset < chunk.length; (startBlock + 1) % 4 ? startBlock += 2: startBlock ++) {
      dataBlocks.push(chunk.slice(offset, offset += blockLength))
    }
  }
}
